(function(){
    ...
    const translations = {
        en: {
            pageTitle: 'Barricade - Dark Mode',
            aboutBtn: 'About',
            langBtn: 'فارسی',
            aboutTitle: 'About Us',
            aboutText: 'Barricade is an offline, pass-and-play strategy board game where players race to reach the opposite side while placing walls to slow each other down.',
            aboutCreatorLabel: 'Created by',
            aboutCreatorName: 'Mohammad hossein shamsi',
            aboutClose: 'Close',
            startSubtitle: 'Offline pass-and-play — choose a mode',
            mode2pTitle: '2 Player',
            mode2pDesc: '1v1 — each player has 10 walls, first to reach the opposite side wins',
            mode4pTitle: '4 Player (2v2 Teams)',
            mode4pDesc: 'Two teams of two — each player has 10 walls, get both teammates to the finish',
            move: 'Move',
            ...
            teamA: 'Team A (Red/Blue)',
            teamB: 'Team B (Green/Yellow)',
            players: {
        ...
        fa: {
            pageTitle: 'باریکید - حالت تیره',
            aboutBtn: 'درباره ما',
            langBtn: 'English',
            aboutTitle: 'درباره ما',
            aboutText: 'باریکید یک بازی فکری آفلاین و نوبتی است که در آن بازیکنان باید زودتر از بقیه به سمت مقابل برسند و در همین حین با گذاشتن دیوار مسیر حریف را کندتر کنند.',
            aboutCreatorLabel: 'سازنده',
            aboutCreatorName: 'محمدحسین شمسی',
            aboutClose: 'بستن',
            startSubtitle: 'یک بازی آفلاین روی یک گوشی، به‌صورت نوبتی',
            mode2pTitle: 'دو نفره',
            mode2pDesc: 'یک به یک — هرکس ۱۰ دیوار دارد، اول کسی که به خط مقابل برسد برنده است',
            mode4pTitle: 'چهار نفره (دو تیم دونفره)',
            mode4pDesc: 'دو تیم دونفره — هر بازیکن ۱۰ دیوار دارد، هر تیم باید هر دو یارش را به انتها برساند',
            move: 'حرکت',
            horizontal: 'دیوار افقی',
            vertical: 'دیوار عمودی',
            undo: 'واگرد',
            repeat: 'تکرار',
            resign: 'انصراف',
    ...
    function setLanguage(lang) {
        ...
        if (appEl.classList.contains('visible')) {
            ...
            updateHistory();
            infoMode.textContent = gameMode === '2p' ? t('mode2p') : t('mode4p');
            infoObjective.textContent = gameMode === '2p' ? t('objective2p') : t('objective4p');
        }
    }
    ...
    function applyStaticTranslations() {
        ...
        setTextContent('lang-btn-label', t('langBtn'));
        ...
        setTextContent('btn-about-close', t('aboutClose'));
        setTextContent('start-subtitle', t('startSubtitle'));
        setTextContent('mode-2p-title', t('mode2pTitle'));
        setTextContent('mode-2p-desc', t('mode2pDesc'));
        setTextContent('mode-4p-title', t('mode4pTitle'));
        setTextContent('mode-4p-desc', t('mode4pDesc'));
    ...
    document.getElementById('btn-lang-toggle').onclick = () => setLanguage(currentLang === 'en' ? 'fa' : 'en');
    ...
    function setup2P() {
        players = [
            { id: 0, name: 'Player 1 (Red)', customName: currentNames.p1, color: '#ff3b30', colorClass: 'red', row: 8, col: 4, walls: 10, target: 'row0', team: 0, finished: !1 },
            { id: 1, name: 'Player 2 (Blue)', customName: currentNames.p2, color: '#007aff', colorClass: 'blue', row: 0, col: 4, walls: 10, target: 'row8', team: 1, finished: !1 },
        ];
        turnOrder = [0, 1];
    }
    ...
    function setup4P() {
        players = [
            { id: 0, name: 'Red', customName: currentNames.red, color: '#ff3b30', colorClass: 'red', row: 0, col: 2, walls: 10, target: 'row8', team: 0, finished: !1 },
            { id: 1, name: 'Blue', customName: currentNames.blue, color: '#007aff', colorClass: 'blue', row: 0, col: 6, walls: 10, target: 'row8', team: 0, finished: !1 },
            { id: 2, name: 'Green', customName: currentNames.green, color: '#34c759', colorClass: 'green', row: 8, col: 2, walls: 10, target: 'row0', team: 1, finished: !1 },
            { id: 3, name: 'Yellow', customName: currentNames.yellow, color: '#ffcc00', colorClass: 'yellow', row: 8, col: 6, walls: 10, target: 'row0', team: 1, finished: !1 },
        ];
        turnOrder = [0, 2, 1, 3];
    }
    ...
    function playerDisplayName(p) {
        ...
        const dict = translations[currentLang].players;
        if (gameMode === '2p') {
            return p.id === 0 ? dict.player1 : dict.player2;
        }
    ...
    function initGame(mode) {
        ...
        wallAnimation = null;
        hideWallConfirm();
        infoMode.textContent = mode === '2p' ? t('mode2p') : t('mode4p');
        infoObjective.textContent = mode === '2p' ? t('objective2p') : t('objective4p');
        renderTopbar();
        updateBtnState();
        updateScores();
        updateActivePlayerUI();
    ...
    document.getElementById('btn-start-2p').onclick = () => showNameEntry('2p');
    document.getElementById('btn-start-4p').onclick = () => showNameEntry('4p');
    ...
    btnRepeat.onclick = () => {
        ...
        if (!confirm(t('confirmRepeat'))) return;
        restartSameGame();
    };
    ...
    btnResign.onclick = () => {
        ...
        if (!confirm(fmt(t('confirmResign'), { name: playerDisplayName(player) }))) return;
        gameOver = !0;
        pendingWallPos = null;
        wallPreviewPos = null;
        hideWallConfirm();
    ...
    function renderTopbar() {
        ...
        if (gameMode === '2p') {
            ...
            vsBox.innerHTML = `<div class="label">${t('match')}</div><div class="amount">${t('vs')}</div>`;
            topbarEl.append(card0, vsBox, card1);
        } else {
            topbarEl.className = 'topbar mode-4p';
    ...
    function createPlayerCard(p) {
        ...
        avatar.className = `avatar ${p.colorClass}`;
        ...
        wallsSpan.id = `p${p.id}-walls`;
        wallsSpan.textContent = p.walls;
        scoreDiv.append(wallsSpan, ' / 10');
        info.appendChild(scoreDiv);
        const bar = document.createElement('div');
        bar.className = 'score-bar';
        const fill = document.createElement('div');
        fill.className = `fill ${p.colorClass}`;
        fill.id = `p${p.id}-fill`;
        fill.style.width = '100%';
        bar.appendChild(fill);
        info.appendChild(bar);
        if (gameMode === '2p' && p.id === 1) info.style.textAlign = 'right';
    ...
    function isTargetCell(player, r, c) {
        switch (player.target) {
            case 'row0': return r === 0;
            case 'row8': return r === 8;
            default: return !1;
        }
    }
    ...
    function startAnimation(oldRow, oldCol, newRow, newCol, player, callback) {
        ...
        animData = { x1: startX, y1: startY, x2: endX, y2: endY, color: player.color, playerId: player.id, progress: 0, callback };
        loopAnimation();
    }

    function animateWall(wallKey, callback) {
        wallAnimation = { row: wallKey.row, col: wallKey.col, isH: wallKey.isH, progress: 0, callback };
        loopAnimation();
    }
    ...
    function draw() {
        ...
        if (!gameOver && uiMode === 'move' && !isAnimating && players.length) {
            const player = currentPlayer();
        ...
        if (!gameOver && (uiMode === 'hwall' || uiMode === 'vwall') && wallPreviewPos && !pendingWallPos && !isAnimating) {
            let pos = wallPreviewPos;
            ctx.fillStyle = 'rgba(255,255,255,0.35)';
            ctx.shadowBlur = 12;
            ctx.shadowColor = 'rgba(255,255,255,0.25)';
            ctx.beginPath();
            if (uiMode === 'hwall') ctx.roundRect(padding + pos.col * cellSize, padding + (pos.row + 1) * cellSize - 4, cellSize * 2, 8, 4);
            else ctx.roundRect(padding + (pos.col + 1) * cellSize - 4, padding + pos.row * cellSize, 8, cellSize * 2, 4);
            ctx.fill();
            ctx.shadowBlur = 0;
        }
        if (!gameOver && pendingWallPos && !isAnimating) {
            ...
            ctx.beginPath();
            if (pos.mode === 'hwall') ctx.roundRect(padding + pos.col * cellSize, padding + (pos.row + 1) * cellSize - 4, cellSize * 2, 8, 4);
            else ctx.roundRect(padding + (pos.col + 1) * cellSize - 4, padding + pos.row * cellSize, 8, cellSize * 2, 4);
            ctx.fill();
            ctx.shadowBlur = 0;
        }
        ...
        for (let r = 0; r < 9; r++) {
            for (let c = 0; c < 9; c++) {
                if (hWalls[r][c]) {
                    let wp = 1;
                    if (wallAnimation && wallAnimation.isH && wallAnimation.row === r && wallAnimation.col === c) wp = wallAnimation.progress;
                    ctx.globalAlpha = wp;
                    ctx.beginPath();
        ...
        ctx.globalAlpha = 1;
        ctx.shadowBlur = 0;
        for (const p of players) drawPiece(p, animData && animData.playerId === p.id);
    }
    ...
    function drawPiece(player, isAnimatingPiece) {
        ...
        ctx.shadowBlur = player.finished ? 22 : 15;
    ...
    function confirmWall() {
        ...
        try {
            if (pos.mode === 'hwall') {
                ...
                // FIX: check perpendicular walls correctly (original used &&, now uses ||)
                if (vWalls[row][col] || vWalls[row][col + 1]) {
                ...
                const row = pos.row, col = pos.col;
                if (row < 0 || row > 7 || col < 0 || col > 7) return;
                // FIX: check perpendicular walls correctly
                if (hWalls[row][col] || hWalls[row + 1][col]) {
                    showToast(t('alertPerpendicular'));
                    pendingWallPos = null;
                    wallPreviewPos = null;
                    hideWallConfirm();
    ...
    function showGameOverDialog(winnerPlayer, loserPlayer, winnerTeam) {
        let winnerLabel, loserLabel, winnerColorClass;
        if (winnerTeam !== undefined && winnerTeam !== null) {
            const loserTeam = winnerTeam === 0 ? 1 : 0;
            winnerLabel = teamName(winnerTeam);
            loserLabel = teamName(loserTeam);
            winnerColorClass = players.find(p => p.team === winnerTeam).colorClass;
        } else {
    ...
    btnGoRepeat.onclick = restartSameGame;
    ...
    function updateActivePlayerUI() {
        ...
        if (player) {
            ...
            infoWalls.textContent = `${player.walls} / 10`;
        }
    }
    ...
    function updateScores() {
        ...
        const player = currentPlayer();
        if (player) infoWalls.textContent = `${player.walls} / 10`;
    }
    ...
    function updateHistory() {
        ...
        if (history.length === 0) {
            ...
            document.querySelector('.history-pagination').innerHTML = `<span>‹</span> 1 / 1 <span>›</span>`;
            return;
        }
        const totalPages = Math.ceil(history.length / ITEMS_PER_PAGE);
        if (currentPage >= totalPages) currentPage = totalPages - 1;
        ...
        const groupSize = gameMode === '2p' ? 2 : 4;
        ...
        for (let i = 0; i < displayHistory.length; i++) {
            ...
            if ((startIndex + i) % groupSize === 0) {
                numSpan.textContent = (Math.floor((startIndex + i) / groupSize) + 1) + '.';
            }
        ...
        if (currentPage === 0) {
            ...
            prevBtn.onclick = () => {
                if (currentPage > 0) { currentPage--; updateHistory(); }
            };
        }
        if (currentPage === totalPages - 1) {
            ...
            nextBtn.onclick = () => {
                if (currentPage < totalPages - 1) { currentPage++; updateHistory(); }
            };
        }
    }
    ...
    function showWallConfirm(pos) {
        ...
        const screenX = (canvasRect.left - wrapperRect.left) + cx * scaleX;
        const screenY = (canvasRect.top - wrapperRect.top) + cy * scaleY;
    ...
    function getCellFromEvent(e) {
        ...
        let x = (e.clientX - rect.left) * scaleX - padding;
        let y = (e.clientY - rect.top) * scaleY - padding;
        if (x < 0 || y < 0 || x > 9 * cellSize || y > 9 * cellSize) return null;
        return { x, y };
    }

    function getWallSnap(e) {
    ...
    function handleWallTap(e) {
        const snap = getWallSnap(e);
        if (!snap) {
            if (pendingWallPos) {
                pendingWallPos = null;
                hideWallConfirm();
        ...
        if (!pendingWallPos || pendingWallPos.row !== snap.row || pendingWallPos.col !== snap.col || pendingWallPos.mode !== snap.mode) {
            pendingWallPos = snap;
            wallPreviewPos = snap;
    ...
    canvas.addEventListener('touchmove', (e) => {
        ...
        if (uiMode !== 'move' && !pendingWallPos) {
            wallPreviewPos = getWallSnap(touch) || null;
            draw();
        }
    }, { passive: !1 });
